...
def send_loop(panda: Panda, packer: CANPacker, car_state: CarState, can_parsers: dict,
    ...
    try:
        while not stop_event.is_set():
            ...
            if frame % 2 == 0:
                # ADAS_LKA @ 50Hz
                lka_addr, lka_data, _ = create_lka_control(
                    packer, CP, frame // 2, lat_active
                )
                panda.can_send(lka_addr, lka_data, VINFAST_CHASSIS_BUS)
...
# def send_loop(panda: Panda, packer: CANPacker, rx_debug: bool = False):
#     """
#     Main control loop:
#       - EPS_LATE_CON (0x37A) @ 100Hz on chassis bus
#       - ADAS_LKA      (0x132) @ 20Hz on camera bus
...
#     frame = 0
#     CP = None  # Not used by vinfastcan helpers, keep for API compatibility
...
#                 packer, CP, frame, apply_angle, lat_active
#             )
#             panda.can_send(eps_addr, eps_data, VINFAST_CAMERA_BUS)
...
#                     packer, CP, frame // 2, lat_active
#                 )
#                 panda.can_send(lka_addr, lka_data, VINFAST_CAMERA_BUS)
...
#                     standstill,
#                 )
#                 panda.can_send(acc_addr, acc_data, VINFAST_CAMERA_BUS)

#             # # Optional: read CAN for debugging
#             # for msg in panda.can_recv():
#             #     if len(msg) == 4:
#             #         address, _, data, bus = msg
#             #     else:
#             #         address, bus, data = msg
#             #     if address in (0x37A, 0x132) or frame % 500 == 0:
#             #         print(f"[CAN RX] ID: 0x{address:X}, Bus: {bus}, Data: {bytes(data).hex()}")
...
#                     else:
#                         address, bus, data = msg
#                     if address in (0x37A, 0x132, 0x32D) or frame % 500 == 0:
#                         print(
#                             f"[CAN RX] ID: 0x{address:X}, Bus: {bus}, Data: {bytes(data).hex()}"
#                         )
...
#             elif frame % 100 == 0:
#                 print(f"[WARN] Loop running slow: {elapsed*1000:.2f} ms")
...
def main():
    ...
    try:
        ...
        print(f"[MAIN] Setting safety mode: {args.safety}")
        ...
        # Run send loop
        send_loop(panda, packer, car_state, can_parsers,
